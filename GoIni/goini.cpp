...
char *GoIni::getCString(const char *section_name, const char *key_name,
  ...
  while (file.getline(line, sizeof(line))) {
    ...
    if (*start == ';' || *start == '#') {
      ...
      if (*end != ']') {
        ...
        setError(error + " is not a valid section name. ] is missing. See "
                         "online documentation of GoIni.",
                 track);
        return _strdup(default_value);
      } else {
        *end = '\0';
        ...
        *end = '\0';
        ...
        if (!strcmp(section, section_name) && !strcmp(name, key_name)) {
          ...
          if (!strcmp(section, section_name))
            this->error = "In File " + this->file_name + " in section " +
                          section_name + " no key named " + key_name +
                          " found.";
          else
            this->error = "In File " + this->file_name + " no section named " +
                          section_name + " found.";
        }
      }
    }
...
std::string GoIni::getString(const char *section_name, const char *key_name,
                             const std::string &default_value) {
  std::string value = getCString(section_name, key_name, default_value.c_str());
  return value;
}

long GoIni::getInt(const char *section_name, const char *key_name,
                   int default_value) {
  std::stringstream buffer;
  buffer << default_value;
  std::string valuestr =
      getCString(section_name, key_name, buffer.str().c_str());
...
bool GoIni::getBoolean(const char *section_name, const char *key_name,
                       bool default_value) {
...
double GoIni::getDouble(const char *section_name, const char *key_name,
...
std::string GoIni::getError() const { return this->error; }

void GoIni::donotAllowComments() { this->GO_INI_ALLOW_INLINE_COMMENTS = false; }
...
char *GoIni::findCharsOrComments(const char *s, const char *chars) {
  if (this->GO_INI_ALLOW_INLINE_COMMENTS) {
    ...
    while (*s && (!chars || !strchr(chars, *s)) &&
           !(was_space && strchr(GO_INI_COMMENTS_PREFIXS, *s))) {
...
void GoIni::setError(const std::string &error, int line_num) {
  ...
  buffer << line_num;
  this->error =
      "In File " + this->file_name + " At Line " + buffer.str() + " " + error;
  return void();
}
